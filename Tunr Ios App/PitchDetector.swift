...
final class PitchDetector: ObservableObject {
    ...
    private func process(signal: [Float]) {
        ...
        guard rms > 0.005 else {
            // Quiet — clear display after a short delay
            let now = CACurrentMediaTime()
            if now - lastPublishTime > 0.8 {
                DispatchQueue.main.async {
                    if self.frequency > 0 {
                        self.frequency = 0
                        self.noteName = "--"
        ...
        // YIN pitch detection
        let detectedFrequency = yinPitch(signal: windowed)
    ...
    private func yinPitch(signal: [Float]) -> Double {
        let n = signal.count
        let halfLen = n / 2
        let minLag = Int(sampleRate / 400)   // ~400 Hz max
        let maxLag = min(Int(sampleRate / 60), halfLen) // ~60 Hz min
        ...
        // Fallback: if nothing below threshold, pick global minimum in range
        if bestTau == 0 {
            ...
            guard minVal < 0.5 else { return 0 }
        }
